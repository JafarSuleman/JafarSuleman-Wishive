...
class CommentScreen extends StatefulWidget {
  ...
  final bool fromPostDetailsScreen;

  CommentScreen({required this.postId, this.showAppBar = true, this.fromPostDetailsScreen = false,  this.openInBottomSheet = false, List<CommentModel> ?commentList}): commentList = commentList??<CommentModel>[];
...
class _CommentScreenState extends State<CommentScreen> {
  ...
  void initState() {
    ...
    future = getCommentsList(commentList.isEmpty);
    super.initState();
    afterBuildCreated(() {
  ...
  Widget commentStackSection(){
    return Stack(
      alignment: Alignment.topCenter,
      children: [
        FutureBuilder<List<CommentModel>>(
          future: future,
          initialData: widget.commentList.isNotEmpty ? widget.commentList : null,
          builder: (ctx, snap) {
            if (snap.hasError) {
              return NoDataWidget(
            ...
            if (snap.hasData) {
              if (snap.data.validate().isEmpty) {
                ...
                return AnimatedListView(
                  ...
                  padding: EdgeInsets.fromLTRB(16, 16, 16, widget.showAppBar ? 110 : 10),
                    ...
                    return Column(
                      children: [
                        CommentComponent(
                          ...
                          isParent: true,
                          openInBottomSheet: widget.openInBottomSheet,
                          comment: _comment,
                          fromPostDetailsScreen: widget.fromPostDetailsScreen,
                          postId: widget.postId,
                          onDelete: () {
                          ...
                          onReply: () async {
                            if(widget.fromPostDetailsScreen){
                              if(widget.openInBottomSheet){
                                if(!widget.fromPostDetailsScreen){
                                  finish(context);
                                }
                                openGeneralBottomSheet(
                                  context: context,
                                  widgetToShow: CommentReplyScreen(
                                    fromPostDetailsScreen: false,
                                    initialTextFieldFocus: true,
                                    callback: () {
                                      mPage = 1;
                                      isChange = true;
                                      future = getCommentsList();
          ...
          Observer(
            builder: (_) {
              if (appStore.isLoading) {
                return Positioned(
                  bottom: mPage != 1 ? widget.showAppBar ? 70 : 10 : null,
