...
Future<List<PostModel>> getWishListPost({int page = 1, int? userId, int? groupId, required String type}) async {
  dynamic a = await handleResponse(await buildHttpResponse('${APIEndPoint.wishListPosts}?${userId!=null ? 'user_id=$userId&':''}type=$type&page=$page&per_page=$PER_PAGE${groupId!=null ? '&group_id=$groupId':''}', method: HttpMethod.GET,));
  return List<PostModel>.from(a['data'].map((e) => PostModel.fromJson(e))).toList();
}
